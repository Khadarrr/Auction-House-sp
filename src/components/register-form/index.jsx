...
const SignUpForm = () => {
  ...
  const validateForm = () => {
    ...
    if (currentStep === 0 && (!username || username.length < 2)) {
  ...
  return (
    <div className="flex items-center justify-center h-screen card glass">
      <div className=" glass w-full max-w-md p-4 rounded-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-4" action="/profile">
            ...
            {currentStep === steps.length - 1 ? (
              <Link to="/login" className="w-full block">
                <button
                  type="submit"
                  className="w-full px-4 py-2 my-2 leading-tight shadow-custom"
                >
                  Create Profile
                </button>
              </Link>
